...
def train(queries: list[str, str, list], hint_generator: HyperQO):
    ...
    for epoch in range(1):
        ...
        for idx, (sql, _, _) in enumerate(queries[:]):
            (pg_plan_time, pg_latency,
             ...
             chosen_leading_pairs) = hint_generator.optimize(sql)
...
def main():
    ...
    train(load_json(config.INPUT_WORKLOAD_NAME), hint_generator=HyperQO(tree_net=tree_net,
                                                                        sql2vec=SQLEncoder(),
